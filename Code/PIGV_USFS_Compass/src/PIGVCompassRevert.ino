...
void setup()
{
  ...
  { // this was originally in the USFSMAX.begin but I moved it here so I can have some control over it
    ...
    {
      //usfsmax.use2DDhiCorrector=false; 
...
void loop()
{
  ...
  { // runs twice per second (or whatever you set the serial update period to)

    // Serial.println(gyroCalActive);
    // click(2);
    if (USE_2D_DHI_CORRECTOR == true && ((angles[0] > 10 || angles[0] < -10) || (angles[1] > 10 || angles[1] < -10)))
    { // 2D calibration values are discarded if the tilt is over 10 degrees, so this tells you
      Serial.println("Tilt!");
      tilt = 1;
    }
    else
  ...
  if (buttonState == 0)
  { // hold the encoder button for 10 seconds to reset the DHI and start a calibration
...
static void serialInterfaceHandler()
{
  ...
  if (serialAvailable())
    serial_input = serialGetCharacter();
  if (serial_input == '1')
  { // Type '1' to initiate usfsmax Gyro Cal
    usfsmax.gyroCal();
    gyroCalActive = true;
  }
...
void showHeadingNumbers(int headingToShow)
{
  ...
  { // I know heading will never be more than 3 digits, but if you want to show different numbers you can up to 99999
  ...
  for (int i = 1; i < 9; i++)
  { // finds the last digit to replace with menuReminder bitmap
    if (asciiInt[i] == -16)
    {
      lastDigit = i;
      break;
  ...
  { // these change the units to thing to remind you what you're doing
  case 0:
  ...
  for (int j = 1; j <= 11; j++)
  { // this actually adds the heading stuff to the display buffer
    ...
    if (asciiInt[j] == 1)
      rowCount -= 1; // this is because the digit 1 is narrower than the rest and the spacing was bugging me //#keming

    if (asciiInt[j + 1] == -16)
    { // this adds space between the heading and the compass
      if (menuReminderAscii != 78 && menuReminderAscii != 82)
      { // different spacings looked right depending on the menuReminder
        displayBuffer[(scrollValue + rowCount) % 360] = 0b11111111;
...
{ // this function writes data to the display
  ...
  if (displayedMenu == 0)
  {
    ...
    if (scroll < 0)
      scroll += 360; // Allow for under|overflow
    if (scroll >= 360)
      scroll -= 360;
